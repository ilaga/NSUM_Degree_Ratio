...
f.datalik<-function(data, v.alpha, v.beta, v.eta, v.etak, v.z, v.muk){
  ...
  # v.theta<-acos(v.z%*%t(v.muk))
  v.theta<-v.z%*%t(v.muk)
  v.null<-exp(outer(v.alpha, v.beta, "+"))
  v.pop<-cp.fcn(v.eta)
  cp.etak = cp.fcn(v.etak)
  v.lambda = matrix(NA, nrow = nrow(v.theta), ncol = ncol(v.theta))
  for(ind in 1:nrow(v.lambda)){
    v.lambda[ind,] = v.null[ind,] * v.pop * cp.etak/(cp.fcn(.000001)*cp.fcn(sqrt((v.eta^2)+(v.etak^2)+(2*v.eta*v.etak*v.theta[ind,]))))
  }
  # v.distr<-cp.fcn(v.etak)/(cp.fcn(.000001)*cp.fcn(sqrt((v.eta^2)+(v.etak^2)+(2*v.eta*v.etak*v.theta))))
  # v.lambda<-v.null*v.pop*v.distr
  
  prob.y<- ((-v.lambda)+(data*log(v.lambda)))
  return(prob.y)
}
...
f.update<-function(data, v.alpha, v.beta, v.eta, v.etak, v.z, v.muk, a.mu, a.s, b.mu, b.s, ad.jump, bd.jump,eta.jump,etak.jump,muk.jump,z.jump,k.mu,k.s,vjind, total.prop,muk.fix,ls.dim){
  ...
  prob.alpha.diff<- rowSums(lik.star, na.rm=T)+dnorm(a.star, a.mu, sqrt(a.s), log = T)-rowSums(lik.old, na.rm=T)-dnorm(a.old, a.mu, sqrt(a.s), log = T)
  ...
  prob.beta.diff<-colSums(lik.new, na.rm=T)+dnorm(b.new, b.mu, sqrt(b.s), log = T)-colSums(lik.old, na.rm=T)-dnorm(b.old, b.mu, sqrt(b.s), log = T)
  ...
  ### renormalize
  # const<-log(sum(exp(v.beta[1:7]))/total.prop)
  
  if(1){ ## Perform scaling
    const<-log(sum(exp(v.beta[n.group.known]))/total.prop)
  ...
  # eta.star<-abs(v.eta+rnorm(1,0, sqrt(0.1)))
  eta.old<-v.eta
  ...
  prob.eta.diff<-sum(lik.star, na.rm=T)+dgamma(eta.star, .5,.5, log = T)-sum(lik.old, na.rm=T)-dgamma(eta.old, .5,.5, log = T)
  ...
  etak.new<-abs(v.etak+rnorm(length(v.etak),0, sqrt(etak.jump)))
  etak.new[etak.new>250]<-abs(250-abs(etak.new[etak.new>250]-250))
  lik.old<- f.datalik(data, v.alpha, v.beta, v.eta, v.etak, v.z, v.muk)
  etak.old<-v.etak
  v.etak<-etak.new
  lik.new<- f.datalik(data, v.alpha, v.beta, v.eta, v.etak, v.z, v.muk)
  # prob.etak.diff<-colSums(lik.new, na.rm=T)+dgamma(etak.new,5,.1, log = T)-colSums(lik.old, na.rm=T)-dgamma(etak.old,5,.1, log = T)
  prob.etak.diff<-colSums(lik.new, na.rm=T)-colSums(lik.old, na.rm=T)
  ...
  for(mm in 1:nrow(z.star)){
    ...
    # z.star[mm,] = rmovMF(n=1, theta = 0.00001 * c(1, 1, 1), alpha = 1)
  }
  z.old<-v.z
  lik.old<- f.datalik(data, v.alpha, v.beta, v.eta, v.etak, v.z, v.muk)
  lik.star<- f.datalik(data, v.alpha, v.beta,v.eta, v.etak, z.star, v.muk)
  prob.z.diff<- rowSums(lik.star, na.rm=T) -
    # dmovMF(z.star, theta = 0.00001 * c(1, 1, 1), alpha = 1, log = T) -
    rowSums(lik.old, na.rm=T)
    # dmovMF(v.z, theta = 0.00001 * c(1, 1, 1), alpha = 1, log = T)
  ...
  for(m in 1:nrow(muk.new)){
    if(m %in% muk.fix.ind){
      ...
      # muk.new[m,] = rmovMF(n=1, theta = 0.00001 * c(1, 1, 1), alpha = 1)
      muk.new[m,] = sphere.update(u.mean = muk.old[m,], u.k = muk.jump[m], m = ls.dim, rot = T)
    }
  }
  
  for(i.fix in 1:m.fix){
    muk.new[muk.fix.ind[i.fix],]<-muk.fix[i.fix,]
  }
  
  lik.old<-f.datalik(data, v.alpha, v.beta, v.eta, v.etak, v.z, muk.old)
  lik.star<-f.datalik(data, v.alpha, v.beta, v.eta, v.etak, v.z, muk.new)
  prob.muk.diff <- colSums(lik.star) -
    # dmovMF(muk.new, theta = 0.00001 * c(1, 1, 1), alpha = 1, log = T) - 
    colSums(lik.old)
    # dmovMF(muk.old, theta = 0.00001 * c(1, 1, 1), alpha = 1, log = T)
  
  z.valid<-(!is.infinite(prob.muk.diff))&(!(is.na(prob.muk.diff)))
  jump.new<-rep(0, nrow(v.muk))   
  jump.new[z.valid]<-rbinom(sum(z.valid), 1, exp(pmin(prob.muk.diff[z.valid], 0)))
  v.muk[which(jump.new==1),]<-muk.new[which(jump.new==1),]
  
  muk.p<-rep(0, nrow(v.muk))
  muk.p[z.valid] = exp(pmin(prob.muk.diff[z.valid], 0))
  
  # v.muk=muk.new
  for(i.fix in 1:m.fix){
    v.muk[muk.fix.ind[i.fix],]<-muk.fix[i.fix,]
  }
...
f.metro<-function(data, n.iter=3000, m.iter=3, n.thin=10,ls.dim=3,total.prop,z.pos.init,muk.fix){
  ...
  alpha.init = log(N * rowSums(mc.data[,known.ind]) / sum(known.sizes[known.ind]))
  beta.init = log(colSums(mc.data) / sum(exp(alpha.init)))
  for(m in 1:m.iter){
    ## initialization
    # mc.v.alpha<-rnorm(data.n,log(50),2)
    # mc.v.beta<-rnorm(data.j,log(.001),1)
    mc.v.alpha = alpha.init
    mc.v.beta = beta.init
    ...
    for(i.fix in 1:m.fix){
      mc.v.muk[muk.fix.ind[i.fix],]<-muk.fix[i.fix,]
    }
