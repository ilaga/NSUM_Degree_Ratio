...
N.i = 20000
N.k = 20
# means = diag(seq(0.1, 0.5, length.out = N.k))
means = diag(seq(0.25, 0.5, length.out = N.k))
# means = diag(rep(0.3, N.k))
means = means + 0.05
## Set the next off-diagonal as mean of diagonals
# for(k in 2:N.k){
#   means[k, k - 1] = means[k - 1, k] = (means[k,k] + means[k - 1, k - 1])/2
# }


# means[N.k - 1, N.k] = means[N.k, N.k - 1] = 0.35
