...
weights <- 1 / readRDS("./Data/qvsq_meal_weights.RDS") ## Meal
...
weights <- weights[complete.cases(y)]
...
y <- as.data.frame(y)
weights <- weights / sum(weights) * nrow(nsum)


## Define weights
degrees.loo.est <- matrix(NA, nrow = N.i, ncol = n.known)
ratio.mat <- ratio.mat.weighted <- matrix(NA, nrow = N.i, ncol = n.known)
for (k in 1:n.known) {
  degrees.loo.est[, k] <- N * rowSums(y[, -k]) / sum(known[-k])
  ratio.mat[, k] <- y[, k]
}
for (i in 1:N.i) {
  ratio.mat.weighted[i, ] <- ratio.mat[i, ] / weights[i]
}
...
for (k in 1:n.known) {
  ...
  mle.loo.est[k] <- N * sum(unlist(y[, k]) / weights) / sum(degrees.est / weights)
}
...
slope.melt$name.class <- c(rep("notname", 10), rep("name", 12))

gg.slope <- ggplot(slope.melt, aes(x = value, y = mle.err, label = name, col = name.class)) +
  scale_color_manual(
    values = c("name" = "red", "notname" = "black"),
    name = "Group Type",
    labels = c("Name", "Non-name")
  ) +
...
ggsave(
  "./Figures/Rwanda_meal_slopes_weighted.pdf",
  gg.slope,
  width = 20,
  height = 10,
  units = "in"
...
ggsave(
  "./Figures/Rwanda_all_results_weighted.pdf",
  gg.res,
  width = 20,
  height = 10,
  units = "in"
...
## Without names and without priest
## Slope.scale
(
  Metrics::mape(known.non.names, mle.loo.non.names.est) -
    Metrics::mape(known.non.names, killworth.slope.scale)
...
ggsave(
  "./Figures/Rwanda_meal_non_names_results_weighted.pdf",
  gg.res,
  width = 20,
  height = 10,
  units = "in"
...
ggsave(
  "./Figures/Rwanda_non_priest_results_weighted.pdf",
  gg.res,
  width = 20,
  height = 10,
  units = "in"
