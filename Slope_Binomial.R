...
c.k.2 <- seq(-1 / max(abs(d^(2) - mean(d^(
  2
)))), 1 / max(abs(d^(2) - mean(d^(
  2
)))), length.out = N.k)
c.k.3 <- seq(-1 / max(abs(d^(1 / 2) - mean(d^(
  1 / 2
)))), 1 / max(abs(d^(1 / 2) - mean(d^(
  1 / 2
)))), length.out = N.k)
c.k.4 <- seq(-1 / max(abs(d^(1 / 3) - mean(d^(
  1 / 3
)))), 1 / max(abs(d^(1 / 3) - mean(d^(
  1 / 3
)))), length.out = N.k)
c.k.5 <- seq(-1 / max(abs(d^(1 / 4) - mean(d^(
  1 / 4
)))), 1 / max(abs(d^(1 / 4) - mean(d^(
  1 / 4
)))), length.out = N.k)
c.k.6 <- seq(-1 / max(abs(d^(-1) - mean(d^(
  -1
)))), 1 / max(abs(d^(-1) - mean(d^(
  -1
)))), length.out = N.k)
...
## Scale
ratio.mat <- scale(ratio.mat,
                   center = FALSE,
                   scale = apply(ratio.mat, 2, mean))
...
df.melt.rel.error$name <- factor(df.melt.rel.error$name, levels = names(known))
