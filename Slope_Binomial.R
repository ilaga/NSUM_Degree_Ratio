...
for(k in 1:N.k){
  ...
  # f.mat[,k] = ((1 + (sqrt(d) - mean(sqrt(d))) * c.k.3[k]))
  # f.mat[,k] = (1 + (d^(1/3) - mean(d^(1/3))) * c.k.4[k])
  # f.mat[,k] = (1 + (d^(1/4) - mean(d^(1/4))) * c.k.5[k])
  # f.mat[,k] = (1 + (d^(1/4) - mean(d^(1/4))) * c.k.5[k])
  ...
  # y[,k] = rpois(N.i, d * known[k] / N * f.mat[,k])
  y[,k] = rbinom(N.i, d, known[k] / N * f.mat[,k])
  
  # y[,k] = round(d * known[k] / N * f)
...
# gg.slope = ggplot(slope.melt, aes(x = value, y = mle.err^(2), label = name)) +
...
df.melt.rel.error$name = factor(df.melt.rel.error$name,
                                levels = names(known))
                                # levels = names(known)[order(known)])
